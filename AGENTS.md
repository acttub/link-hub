...
- **홈에는 2~4개만 둔다.** 작은 행이 늘어나면 그 총 무게가 메인 카드를 눌러 깔때기가 옆으로 눕는다.
  5번째가 생기면 홈에 끼워 넣지 말고, 낮은 강조의 "다른 연습도 보기" 한 줄을 붙여 전체 목록
  페이지로 보낸다. 그래야 프로젝트를 버리지 않고도 코어 계층이 유지된다.
  홈에 남길 2~4개는 목적지 referrer 숫자로 정한다(아래 "측정").
