...
- `acttub.com`은 소스 저장소가 특정되지 않아 아직 안 붙었다. **허브에서 코어 카드를 누른 클릭은
  여전히 측정 공백이다** — 목적지가 우리 계측을 안 갖고 있어 역산할 대상이 없다.
- 다만 **우리가 뿌리는 링크의 채널 구분은 `/go` 경유로 해결했다**(README "채널별 유입 기록").
  이건 Vercel이 아니라 구글 시트에 쌓이고, 허브 클릭이 아니라 외부 채널 유입을 센다. 둘을 섞지 않는다.
